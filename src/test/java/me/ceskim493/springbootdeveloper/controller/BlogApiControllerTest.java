...
class BlogApiControllerTest {
    ...
    public void findArticle() throws Exception {
        ...
        // when : 저장한 블로그의 글의 id값을 API로 호출하고
        final ResultActions resultActions = mockMvc.perform(get(url, savedArticle.getId()));

        // then : 응답코드가 200이고, 반환받은 content와 title이 저장된 값과 같은지 확인한다.
        resultActions
                .andExpect(status().isOk())
    ...
    public void deleteArticle() throws Exception {
        ...
        // when : 저장한 블로그의 글의 id값으로 삭제 API로 호출하고
        mockMvc.perform(delete(url, savedArticle.getId()))
                        .andExpect(status().isOk());

        // then : 응답코드가 200이고, 블로그 글 리스트를 조회해 조회한 배열의 크기가 0인지 확인한다.
        List<Article> articles = blogRepository.findAll();
    ...
    public void updateArticle() throws Exception {
        ...
        // then : 응답코드가 200이고, 블로그 글 id로 조회한 후에 값이 수정되었는지 확인한다.
